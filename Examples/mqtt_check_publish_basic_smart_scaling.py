...
from publish_burst import my_publish_burst
...
async def main():
    ...
    async def sample_handler(topic: str, payload: str):
        ...
        logger.info(f"Received message in sample handler on topic: {topic} with payload: {payload}")

    try :
        await mqtt_client_connector.connectAsync(username=test_client.user_name, password=test_client.password)
        await mqtt_client_connector.subscribeAsync(topic="my_own_topic", handler=sample_handler, qos=2)
        ...
        await mqtt_client_connector.publishAsync(topic="my_own_topic", payload="Hello from mqtt_check_publish_basic.py",
                                                 qos=2)
        burst_task = asyncio.create_task(my_publish_burst())
        ...
        # logger.exception(me)
        return
    except MyMqttSubscriptionError as mse:
        logger.exception(f"Unable to subscribe to the topic - my_own_topic: {mse}")
